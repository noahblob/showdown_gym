...
class ShowdownEnvironment(BaseShowdownEnv):
    ...
    def hint_action(self, battle: AbstractBattle):
        ...
        # Helper to score a move with the same formula used for action selection
        def score_move(move_type, base_power, attacker_types, defender_types, opp_ability=None, acc=100):
            ...
            if is_immune_ability(move_type, opp_ability):
                return 0.0, 0.0
            # Type effectiveness
            eff = calc_effectiveness(move_type, defender_types)
            if eff == 0.0:
                return 0.0, 0.0
            # STAB
            ...
            # Accuracy
            acc_mult = (acc / 100.0) if acc else 1.0
            total = base_power * eff * stab * weather_mult * terrain_mult * acc_mult
            return total, eff
        ...
        for i, move in enumerate(battle.available_moves):
            ...
            total_score, type_effectiveness = score_move(
                move_type=move_type,
        ...
        # Evaluate potential switches using the same move-scoring logic
        ...
        for i, mon in enumerate(team_list):
            if i >= 6:
                break
            # Only consider available, non-fainted switches and not the active mon
            if mon == me or mon.fainted or mon not in battle.available_switches:
                continue

            switch_types = mon.types
            # Try to use revealed moves if available; otherwise, estimate using STAB types
            candidate_scores = []
            # Use known moves if present
            try:
            ...
            if known_moves:
                for m in known_moves:
                    ...
                    m_power = getattr(m, 'base_power', 0) or 0
                    m_acc = getattr(m, 'accuracy', 100) or 100
                    if m_type is None or m_power <= 0:
                        continue
                    score, _ = score_move(
                        move_type=m_type,
                        base_power=m_power,
                        attacker_types=switch_types,
                        defender_types=opp_types,
                        opp_ability=opp_ability,
                        acc=m_acc,
                    )
                ...
                # Estimate with generic STAB moves for each of the mon's types (assume 80 BP and 100 acc)
                for t in switch_types:
                    if t is None:
                        continue
                    score, _ = score_move(
                        move_type=t,
                        base_power=80,  # generic estimate
                        attacker_types=switch_types,
                        defender_types=opp_types,
                        opp_ability=opp_ability,
                        acc=100,
            ...
            if candidate_scores:
                switch_score = max(candidate_scores)
                if switch_score > best_switch_score:
                    best_switch_score = switch_score
                    best_switch_idx = i
        
        # Decision logic: prefer the action (move vs switch) with the higher score
        if best_switch_idx is not None and best_switch_score > best_move_score:
