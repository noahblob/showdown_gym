...
class ShowdownEnvironment(BaseShowdownEnv):
    ...
    def __init__(
        ...
        self._ability_index = self._build_ability_index()
        self._item_index = self._build_item_index()
        self._move_index = self._build_move_index()
        self._types = self._build_type_list()
    ...
    def _build_species_index(self) -> Dict[str, int]:
        names: Set[str] = set()
        pokedex = getattr(self._gen_data, "pokedex", {})
        if isinstance(pokedex, dict):
            self._update_name_set(names, pokedex.keys())
            for entry in pokedex.values():
                self._update_name_set(names, entry.get("baseSpecies"))
                self._update_name_set(names, entry.get("species"))
        else:
            self._update_name_set(names, pokedex)
        self._update_name_set(names, getattr(self._gen_data, "species", None))
        return self._create_index_from_names(names, default="unknown")

    def _build_ability_index(self) -> Dict[str, int]:
        names: Set[str] = set()
        self._update_name_set(names, getattr(self._gen_data, "abilities", None))
        pokedex = getattr(self._gen_data, "pokedex", {})
        if isinstance(pokedex, dict):
            for entry in pokedex.values():
                self._update_name_set(names, entry.get("abilities"))
        return self._create_index_from_names(names, default="unknown_ability")

    def _build_item_index(self) -> Dict[str, int]:
        names: Set[str] = set()
        self._update_name_set(names, getattr(self._gen_data, "items", None))
        pokedex = getattr(self._gen_data, "pokedex", {})
        if isinstance(pokedex, dict):
            for entry in pokedex.values():
                for key in (
                    ...
                    self._update_name_set(names, entry.get(key))
        default_item = getattr(GenData, "UNKNOWN_ITEM", "unknown_item")
        return self._create_index_from_names(names, default_item)

    def _build_move_index(self) -> Dict[str, int]:
        names: Set[str] = set()
        moves_attr = getattr(self._gen_data, "moves", None)
        if isinstance(moves_attr, dict):
            self._update_name_set(names, moves_attr.keys())
        else:
            self._update_name_set(names, moves_attr)
        default_move = "unknown_move"
        names.add(default_move)
        return self._create_index_from_names(names, default_move)

    def _build_type_list(self) -> List[str]:
        chart = getattr(self._gen_data, "type_chart", {})
        if isinstance(chart, dict) and chart:
            types = sorted({str(type_name).upper() for type_name in chart.keys()})
        else:
            types = []
        if len(types) != len(self._DEFAULT_TYPES):
            types = [type_name.upper() for type_name in self._DEFAULT_TYPES]
        return types

    def _update_name_set(self, target: Set[str], value: Any) -> None:
        for name in self._collect_names(value):
    ...
    def _collect_names(value: Any) -> List[str]:
        ...
        if isinstance(value, dict):
            ...
            for sub_value in value.values():
                names.extend(ShowdownEnvironment._collect_names(sub_value))
            return names
        if isinstance(value, IterableABC):
            names: List[str] = []
            for element in value:
                names.extend(ShowdownEnvironment._collect_names(element))
            return names
        return [str(value)]

    @staticmethod
    def _create_index_from_names(names: Set[str], default: str) -> Dict[str, int]:
        normalized_default = str(default).strip().lower()
        normalized_names = {
            str(name).strip().lower() for name in names if str(name).strip()
        }
        normalized_names.discard(normalized_default)
        ordered = [normalized_default] + sorted(normalized_names)
    ...
    def _gender_one_hot(self, mon) -> np.ndarray:
        ...
        g = self._normalize_gender(getattr(mon, "gender", None))
        if g.startswith("m"):
    ...
    def _normalize_gender(gender: Any) -> str:
        ...
        for attr in ("value", "name"):
            candidate = getattr(gender, attr, None)
            if candidate:
