...
class ShowdownEnvironment(BaseShowdownEnv):
    ...
    def get_additional_info(self):
        ...
        if self.battle1 is not None:
            ...
            info[agent]["remaining_pokemon"] = sum(1 for mon in battle.team.values() if not mon.fainted)
            info[agent]["opponent_remaining_pokemon"] = sum(1 for mon in battle.opponent_team.values() if not mon.fainted)
            ...
            # Type advantage metrics (calculate type effectiveness)
            if active and opp_active:
                # Calculate defensive type advantage (how well opponent threatens you)
                ...
                info[agent]["defensive_type_disadvantage"] = calc_type_advantage(opp_active.types, active.types)
                info[agent]["offensive_type_advantage"] = calc_type_advantage(active.types, opp_active.types)
    ...
    def embed_battle(self, battle: AbstractBattle) -> np.ndarray:
        onehot = hint_action(battle).astype(np.float32)
        return onehot
